...
public class FileClientUI extends Application {
	...
	private HBox createHeader() {
		...
		// ----------------------------------
		hbox.getChildren().addAll(serverChooser(), siteChooser(), browser,
				upload, clear, barGrid);
	...
	public void start(Stage primaryStage) throws Exception {
		...
		Parent root = BorderPaneBuilder.create().top(header).center(table)
				.bottom(createStatusBar()).build();

		Scene scene = new Scene(root, width, height);

		primaryStage.setScene(scene);
		primaryStage.setTitle("JFX - File Server Client(FSC) - MINA");
