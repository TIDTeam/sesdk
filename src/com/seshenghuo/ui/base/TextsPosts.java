 ...
 * 
 */
public class TextsPosts extends AbstractBase<TextsPostsBean> {
	 ...
	 */
	@Override
	public Response<Object> update(TextsPostsBean bean) {
		// TODO Auto-generated method stub
		String sql = SQL.getSQL(Constant.SQL.SP_UPT_TEXTS);
		...
		try {
			...
			stmt.setString(5, bean.getTextBody());
			...
			L.info(TextsPosts.class, "update()", "INFO", "Result is " + result);
			...
			L.error(TextsPosts.class, "update()", rcode, rmessage);
		} finally {
			db.close(stmt, rs);
			resp.setCode(rcode);
	 ...
	 */
	@Override
	public Response<Object> add(TextsPostsBean bean) {
		// TODO Auto-generated method stub
		String sql = SQL.getSQL(Constant.SQL.SP_INS_TEXTS);
		Connection conn = db.getConnection();
		...
		try {
			...
			stmt.setString(8, bean.getTextBody());
			stmt.setString(9, bean.getTextPostDate());
			...
			L.info(TextsPosts.class, "add()", "INFO", "Result is " + result);
			...
			L.error(TextsPosts.class, "add()", rcode, rmessage);
		} finally {
			db.close(stmt, rs);
			resp.setCode(rcode);
			resp.setMessage(rmessage);
	 ...
	 */
	@Override
	public Response<ArrayList<TextsPostsBean>> query(TextsPostsBean bean, int pageIndex,
			int pageSize) {
		// TODO Auto-generated method stub
		String sql = SQL.getSQL(Constant.SQL.SP_QRY_TEXT);
		Connection conn = db.getConnection();

		int code = DBUtil.UNKNOWN_ERROR;
		String rcode = "";
		String rmessage = "";
		Response<ArrayList<TextsPostsBean>> resp = new Response<ArrayList<TextsPostsBean>>();
		CallableStatement stmt = null;
		ResultSet rs = null;
		ArrayList<TextsPostsBean> list = new ArrayList<TextsPostsBean>();
		;
		TextsPostsBean b = null;
		int recordsize = 0;
		...
		try {
			...
			while (rs.next()) {
				...
				b.setTextId(rs.getInt(9));
				b.setTextBody(rs.getString(10));
				b.setTextPostDate(rs.getString(11));
			...
			L.error(TextsPosts.class, "query()", rcode, rmessage);
		} finally {
			db.close(stmt, rs);
	 ...
	 */
	@Override
	public Response<ArrayList<TextsPostsBean>> query(int uniqueId) {
		// TODO Auto-generated method stub
		TextsPostsBean bean = new TextsPostsBean();
		bean.setPostId(uniqueId);
		Response<ArrayList<TextsPostsBean>> resp = query(bean, 0, 0);
